package io.github.mightguy.cloud.manager.util;

import io.github.mightguy.cloud.manager.exception.ExceptionCode;
import io.github.mightguy.cloud.manager.exception.SolrCommonsException;
import java.io.IOException;
import org.apache.solr.client.solrj.SolrClient;
import org.apache.solr.client.solrj.SolrRequest;
import org.apache.solr.client.solrj.SolrServerException;
import org.apache.solr.client.solrj.response.QueryResponse;

public final class QueryRequestUtils {

  public static QueryResponse getQueryResponse(SolrRequest request, SolrClient solrClient)
      throws SolrCommonsException {
    try {
      return new QueryResponse(solrClient.request(request), solrClient);
    } catch (IOException | SolrServerException ex) {
      throw new SolrCommonsException(ex, ExceptionCode.SOLR_EXCEPTION, "UNABLE TO FETCH DATA");
    }
  }
}
